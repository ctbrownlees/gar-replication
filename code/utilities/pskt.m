function P1=pskt(x,location,scale,shape,df)
%dskt pskt qskt rskt
%Skew-t Distribution
% 
%DESCRIPTION
...
%n  scalar representing the desired sample size. 
% 
%OPTIONAL ARGUMENTS
% 
...
%VALUE
% 
%density (dskt), probability (pskt), quantile (qskt) and  random sample (rskt)
...
%pskt make use of function pmskt  "The multivariate skew-t distribution"
% 
%REFERENCES
%
...
% 
%EXAMPLES
%a   = pskt(1)                     #should give back 0.2420
%b   = pskt(1,0,1,0,Inf)           #should give back 0.2420
%c   = pskt(1,[],[],[],[])         #should give back 0.8413
%d   = pskt()                      #error: Argument x is missing
%e   = pskt([])                    #error: x cannot be an empty vector 
...
P1=zeros(1,length(z));
    for i=1:length(z)
        if round(df)==df
            P1(i)=pmskt(z(i),0,1,shape,df);
        else
            if abs(z(i))==Inf
                P1(i)=(1+sign(z(i)))./2;
            else
                if z(i)==10+50/df
                    P1(i)=int(dskt,-Inf,z(i),shape,df);
                else
                    P1(i)=int(PF,0,Inf,shape,df,t_value==z(i));
                end;
            end;
        end;
    end;

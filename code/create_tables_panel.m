function create_tables_panel( H , covs )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Creates tables 4-6 of the paper in HOME/tables. This files assumes that
% the folder data/output contains the .mat files generated by garch_oos and
% qr_oos. 
%
% INPUTS:
...
% 
% OUTPUTS:
%  This script creates the tables in .csv format in the folder
%  HOME/tables. 
%
...
for h = H   
  for c = 1:length( covs )
    ...
    QRh     = M.QRUNC;
    % QR results
    load( sprintf( '%s/data/output/panel_QR_%.2f_%.0f_ahead.mat' , HOME , 1 - coverage , h ) )
    QR     = M;
    % GARCH results
    ...
    ./ (sum( quantile( gdp_garch , 0.99 ) >= x ) );   % Length
    dq_hits_struct  = @( x ) dq_hits( coverage , x , h ); % Function to evaluate on a struct
    dq_fin_uni   = @( x ) aug_dq( coverage , x , global_nfci , h); % DQ NFCI
    ...
    for k = 1:length(marg)
      for i=1:N
        % DQ UNC
        UUC(k,i)    = dq_unc(coverage, H.( marg{k} )( : , i ) , h );
        % Each country is backtested against its own data
        UDQ(k , i)  = dq_hits(coverage, H.( marg{k} )( : , i ) , h );
        UADQ(k , i) = aug_dq( coverage, H.( marg{k} )( : , i ) , nfci_os( : , i ) , h );
        RADQ(k , i) = aug_dq( coverage, H.( marg{k} )( : , i ) , gdp_dq( : , i ) , h );
     ...
     writetable(TB2out,...
     sprintf('%s/tables/panel/MarginalGaR_%s_%.0f_ahead.csv',...
     ...
     writetable(TB4,...
     sprintf('%s/tables/panel/IMF_%s_%.0f_ahead.csv',...
     HOME , strrep(num2str(1-coverage,'%.2f'),'.',''),h),...
     'WriteRowNames',true)
